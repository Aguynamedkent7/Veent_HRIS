<script lang="ts">
	...
	const navItems = $derived([
		...
		{
			href: '/attendance', label: 'Attendance', show: true,
		...
		{
			href: '/payroll', label: 'Payroll', show: isPayroll,
		...
		{
			href: '/benefits', label: 'Benefits', show: isAdmin,
				...
				<div>
					<button
						...
						class="flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors
							{inSettings ? 'bg-primary/15 text-primary' : 'text-muted-foreground hover:bg-accent hover:text-foreground'}"
					>
						<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
							<path stroke-linecap="round" stroke-linejoin="round" d={settingsIcon} />
						</svg>
						<span class="flex-1 text-left">Settings</span>
						<svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 shrink-0 transition-transform {settingsExpanded ? 'rotate-180' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
							...
							{#each settingsChildren as child (child.href)}
								...
								<a
									...
									class="block rounded-md px-3 py-1.5 text-sm transition-colors
										{childActive ? 'bg-primary/15 font-medium text-primary' : 'text-muted-foreground hover:bg-accent hover:text-foreground'}"
								>
									{child.label}
								</a>
							{/each}
