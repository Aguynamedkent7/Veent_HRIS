...
<div class="mx-auto max-w-2xl space-y-6">
	...
	<div class="flex items-center justify-between">
		...
		<span class="rounded-full px-2.5 py-1 text-xs font-medium {statusClass(req.status)}">{req.status}</span>
	</div>
	...
	<div class="rounded-lg border bg-card p-4">
		<dl class="grid grid-cols-3 gap-y-2 text-sm">
			...
			{#if req.dateFrom}
				...
				<dd class="col-span-2">{formatShortDate(req.dateFrom)}{#if req.dateTo && req.dateTo !== req.dateFrom} – {formatShortDate(req.dateTo)}{/if}</dd>
			{/if}
			{#if req.hours}
	...
	<div class="space-y-3">
		...
		<ol class="space-y-2">
			{#each req.steps as step, i (step.id)}
				...
				<li class="flex items-start gap-3 rounded-lg border p-3 {active ? 'border-primary/50 bg-primary/5' : ''}">
					<div class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-medium
						{step.decision === 'APPROVED' ? 'bg-green-100 text-green-700' : step.decision === 'REJECTED' ? 'bg-red-100 text-red-700' : step.decision === 'RETURNED' ? 'bg-orange-100 text-orange-700' : 'bg-muted text-muted-foreground'}">
						{i + 1}
					</div>
					<div class="min-w-0 flex-1">
						...
						<p class="text-xs text-muted-foreground">
							{#if step.decision}
								{step.decision}{#if step.actor} by {step.actor.email}{/if}{#if step.decidedAt} · {formatShortDate(step.decidedAt)}{/if}
							{:else if active}
