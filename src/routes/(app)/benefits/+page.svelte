...
<div class="space-y-6">
	...
	<!-- Create form -->
	{#if showCreate}
		<form method="POST" action="?/createPlan" use:enhance class="rounded-lg border bg-card p-4 space-y-4">
			<h2 class="font-semibold">New Benefit Plan</h2>
			<div class="grid gap-3 sm:grid-cols-2">
				<div>
					<label class="text-sm font-medium">Name</label>
					<input name="name" required class="mt-1 flex h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" />
				</div>
				<div>
					<label class="text-sm font-medium">Type</label>
					<select name="type" class="mt-1 flex h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
				...
				<div>
					...
					<input name="provider" class="mt-1 flex h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" />
				</div>
				<div>
					<label class="text-sm font-medium">Description</label>
					<input name="description" class="mt-1 flex h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" />
				</div>
				<div>
					<label class="text-sm font-medium">Employee Cost (PHP)</label>
					<input name="employeeCost" type="number" min="0" step="0.01" class="mt-1 flex h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" />
				</div>
				<div>
					<label class="text-sm font-medium">Employer Cost (PHP)</label>
					<input name="employerCost" type="number" min="0" step="0.01" class="mt-1 flex h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" />
				</div>
			</div>
			...
			<tbody class="divide-y">
				{#each data.plans as plan (plan.id)}
					<tr class="hover:bg-muted/30">
						...
						<td class="px-4 py-3 text-right">{plan.employeeCost != null ? formatCurrency(Number(plan.employeeCost)) : '—'}</td>
						<td class="px-4 py-3 text-right">{plan.employerCost != null ? formatCurrency(Number(plan.employerCost)) : '—'}</td>
						<td class="px-4 py-3">
							<span class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium {plan.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}">
				...
				{:else}
					<tr>
						<td colspan="6" class="px-4 py-8 text-center text-muted-foreground">No benefit plans found</td>
					</tr>
				{/each}
			</tbody>
		</table>
