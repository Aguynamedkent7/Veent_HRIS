				...
				{#each data.days as d (d.id)}
					...
					>
						<td class="px-3 py-2 whitespace-nowrap"
							>{fmtDate(d.date)}
							{#if d.isLocked}<span title="locked" class="text-muted-foreground">🔒</span>{/if}</td
						>
						<td class="px-3 py-2"
							><span
								...
								{:else}
									...
									>
										...
										<input
											...
											title="Regular hrs"
											class="h-7 w-16 rounded border border-input bg-background px-1 text-xs"
										/>
										<input
											name="overtimeHours"
											type="number"
										...
										>
											{#each STATUSES as s (s)}<option value={s} selected={s === d.status}
													>{s}</option
												>{/each}
										</select>
										<button
				...
				{:else}
					<tr
						><td
							...
							>No attendance for this range. {#if data.canManage}Click "Derive from punches".{/if}</td
						></tr
					>
				{/each}
			</tbody>
