...
<svelte:head>
	<title>Apply for {data.posting.title} — Veent HRIS</title>
</svelte:head>

<div class="mx-auto max-w-2xl space-y-6 py-8">
	<div class="space-y-1">
		<h1 class="text-2xl font-bold tracking-tight">Apply for {data.posting.title}</h1>
	...
	{#if form?.success}
		<div class="rounded-md border border-green-300 bg-green-50 px-4 py-6 text-center space-y-2">
			...
			<a
				href="/recruitment"
				class="mt-4 inline-block rounded-md border px-4 py-2 text-sm hover:bg-accent"
			>
				View Other Openings
			</a>
		</div>
