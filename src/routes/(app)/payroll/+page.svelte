...
<div class="space-y-6">
	...
	{#if showCreate}
		<form method="POST" action="?/create" use:enhance class="rounded-lg border p-4 space-y-3">
			...
			{#if form?.error}<div class="rounded bg-destructive/10 px-3 py-2 text-sm text-destructive">{form.error}</div>{/if}
			...
			<tbody class="divide-y">
				{#each runs as run (run.id)}
					<tr class="hover:bg-muted/30">
						...
						<td class="px-4 py-3">
							<span class="rounded-full px-2 py-0.5 text-xs font-medium {run.status === 'APPROVED' ? 'bg-green-100 text-green-700' : run.status === 'COMPUTED' ? 'bg-blue-100 text-blue-700' : run.status === 'VOIDED' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}">
								{run.status}
						...
						<td class="px-4 py-3 flex gap-2">
							{#if run.status === 'DRAFT'}
								<form method="POST" action="?/compute" use:enhance>
									...
									<button type="submit" class="text-primary text-xs hover:underline">Compute</button>
								</form>
							{/if}
				...
				{:else}
					<tr>
						<td colspan="6" class="px-4 py-8 text-center text-muted-foreground">No payroll runs yet</td>
					</tr>
				{/each}
			</tbody>
		</table>
