...
import { getReview, saveSelfAssessment, submitManagerReview, acknowledgeReview } from '$lib/server/services/performance'
...
export const actions: Actions = {
	saveSelf: async ({ request, locals, params, getClientAddress }) => {
		...
		return run(() => saveSelfAssessment(params.id, employeeId, text, ctxOf(locals, getClientAddress())))
	},

	submitReview: async ({ request, locals, params, getClientAddress }) => {
		const data = await request.formData()
		const parsed = z
			.object({ managerComments: z.string().optional(), overallRating: z.coerce.number().int().min(1).max(5).optional() })
			.safeParse(Object.fromEntries(data))
		if (!parsed.success) return fail(422, { error: 'Invalid review' })
		const reviewerId = await myEmployeeId(locals.user!.id)
		return run(() => submitManagerReview(params.id, reviewerId, parsed.data, ctxOf(locals, getClientAddress())))
	},
