...
import { updateLoan } from '$lib/server/services/payroll/loans'
...
export const PATCH: RequestHandler = async ({ locals, params, request, getClientAddress }) => {
	...
	try {
		const loan = await updateLoan(params.id, locals.user.organizationId, parsed.data, ctx)
		return json({ data: loan })
	} catch (e: unknown) {
		const err = e as { status?: number; body?: { message?: string } }
		if (err?.status === 404) return apiError(404, err.body?.message ?? 'Loan not found')
		throw e
	}
}
