...
describe('verifyHmac', () => {
	...
	it('accepts a valid, fresh signature', () => {
		expect(
			verifyHmac({ rawBody: BODY, signature, timestamp, secret: SECRET, nowSeconds: now })
		).toEqual({
			valid: true
		})
	})
	...
	it('accepts a timestamp at the edge of the replay window', () => {
		...
		expect(
			verifyHmac({ rawBody: BODY, signature, timestamp, secret: SECRET, nowSeconds: edge }).valid
		).toBe(true)
	})

	it('rejects missing signature / timestamp / secret', () => {
		expect(
			verifyHmac({ rawBody: BODY, signature: null, timestamp, secret: SECRET, nowSeconds: now })
				.valid
		).toBe(false)
		expect(
			verifyHmac({ rawBody: BODY, signature, timestamp: null, secret: SECRET, nowSeconds: now })
				.valid
		).toBe(false)
		expect(
			verifyHmac({ rawBody: BODY, signature, timestamp, secret: undefined, nowSeconds: now }).valid
		).toBe(false)
	})
