...
import { computeEmployeeResult, type EmployeeComputeConfig } from '$lib/server/services/payroll/calculator'
...
describe('computeEmployeeResult (shared run/calculator engine)', () => {
	...
	it('honors taxability from config (BASIC set non-taxable → taxableGross 0)', () => {
		const r = computeEmployeeResult(comp, att({ regularHours: 80 }), {}, cfg({ taxableByCode: new Map([['BASIC', false]]) }))
	...
	it('deducts a loan installment on top of statutory', () => {
		const r = computeEmployeeResult(comp, att({ regularHours: 80 }), {}, cfg({ loans: [{ refId: 'L1', label: 'Loan', installment: 1000, balance: 3000 }] }))
	...
	it('is deterministic for identical inputs (calculator == run guarantee)', () => {
		const a = computeEmployeeResult(comp, att({ regularHours: 80, overtimeHours: 5 }), { allowances: 1000 }, cfg())
		const b = computeEmployeeResult(comp, att({ regularHours: 80, overtimeHours: 5 }), { allowances: 1000 }, cfg())
		expect(a).toEqual(b)
	})
})
