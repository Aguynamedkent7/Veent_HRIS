 ...
 * Hence a static scan.
 *
 * Scoped to the guard families converted in this PR. `requireCapability` (#256 PR 3) and the
 * non-throwing `can(` display flags (#258 PR 4) are added to this pattern as those land, so the
 * test is green at every step rather than red for three PRs.
 *
 ...
 */
const SINGULAR_GUARD = /require(?:MinRole|PayrollManage|PayrollReports)\([^,)]*\.role\b/
