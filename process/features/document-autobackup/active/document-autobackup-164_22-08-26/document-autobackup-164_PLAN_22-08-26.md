...
| ST2 | Crash mid-run leaves a half-written directory that retention counts as good | `manifest.json` is written **last** (AD-008). `runsToPrune` only considers `SUCCESS`/`PARTIAL` rows. Stale `RUNNING` rows older than 12h are flipped to `FAILED` at the start of the next run and their directories removed | `T-U-04`, `M7` |
...
|---|---|---|
| M1 | Sign out. Sign in as an **HR Admin**. Open `/settings` | **Positive control first (E-17):** a tile titled `Company Information` IS present — proving the grid rendered and the selector works. Then: there is **no** tile titled `Document Backup`. Then navigate directly to `/settings/backup`: the page body shows the app's 403 error text `Insufficient permissions` |
...
| M9 | Record `find uploads -type f \| wc -l` first. Then run (fish-valid): `env BACKUP_DIR=$PWD/uploads/bk pnpm exec dotenv -e .env.dev -- tsx scripts/backup-documents.ts` | **Positive assertions (E-17):** stdout/stderr contains `backup destination must not be inside UPLOAD_DIR`; `echo $status` prints non-zero; `find uploads -type f \| wc -l` prints the same number recorded before the step. Then: `ls uploads/` shows no `bk` directory |
...
| M11 | **Concurrency (E-11).** With the marker present, launch two runs in the same second (fish): `env BACKUP_DIR=$PWD/backups pnpm exec dotenv -e .env.dev -- tsx scripts/backup-documents.ts --force &; env BACKUP_DIR=$PWD/backups pnpm exec dotenv -e .env.dev -- tsx scripts/backup-documents.ts --force &; wait` | Exactly one process prints `N file(s) copied`; the other prints `another backup is already running — skipped`. `ls backups/org_veent/ \| wc -l` increased by exactly `1`. `docker exec veent-db-5434 psql -p 5434 -U veent -d veent_hris -c "select count(*) from backup_runs where status='RUNNING';"` returns `0` after both exit |
