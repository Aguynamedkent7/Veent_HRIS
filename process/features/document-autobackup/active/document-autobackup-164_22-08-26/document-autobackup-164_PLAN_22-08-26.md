...
| S3 | Backup directory world-readable — 201 files at rest with looser permissions than `UPLOAD_DIR` | Every `mkdir` in the LOCAL branch uses `{ recursive: true, mode: 0o700 }`; every file written with `{ mode: 0o600 }`. The script logs the effective mode of the destination root at start | `destination.ts` | `T-M-08` (manual, `ls -ld`) |
| S4 | Secrets leaked into logs, the UI, or an error string | Three layers: (a) credentials are read into a local `Destination` object and never logged; (b) every error stored on `BackupRun.error` or printed passes through `sanitizeError(message, [accessKeyId, secretAccessKey, endpoint, bucket, backupDir])` which replaces each occurrence with `[redacted]`; (c) the settings page renders `error` only, and never any env value | `plan.ts`, `run.ts`, `+page.server.ts` | `T-U-09`, `T-M-07` |
| S5 | Unauthorized read/edit of backup config or run history | `requireAnyCapability(user.roles, 'ADMINISTER_SYSTEM')` in `load` **and again inside every form action** — the repo's standing double-guard. `ADMINISTER_SYSTEM` = `SUPER_ADMIN`, `CEO`, documented as *"payroll config, user provisioning, role activation, the settings surface"*, which is exactly what this is. **Not** `OVERRIDE_FINALIZED` — nothing here is irreversible | `src/routes/(app)/settings/backup/+page.server.ts` | `T-M-01`, `T-E-01` |
| S6 | Cross-tenant read — one org's backup containing another's documents | Two independent barriers: the collector queries walk the relation (`employee: { user: { organizationId } }` / `request: { employee: { user: { organizationId } } }`), and the destination path is prefixed with `organizationId`, so a mis-scoped row lands in a visibly wrong directory rather than silently mixing. The run-history query filters on `organizationId` from the session, never from a form field | `run.ts`, `+page.server.ts` | `T-U-10`, `T-M-06` |
...
|---|---|---|---|
| ST1 | Some files unreadable (missing on disk, permission, I/O error) | Recorded in `failed[]` in the manifest and in `BackupRun.failedCount`; run status is **`PARTIAL`**, never `SUCCESS`; admins are notified. The run continues — one bad file must not cost the other 411 | `T-U-03`, `T-M-04` |
| ST2 | Crash mid-run leaves a half-written directory that retention counts as good | `manifest.json` is written **last** (AD-008). `runsToPrune` only considers `SUCCESS`/`PARTIAL` rows. Stale `RUNNING` rows older than 12h are flipped to `FAILED` at the start of the next run and their directories removed | `T-U-04`, `T-M-05` |
| ST3 | Concurrent runs (cron overlapping a long run; an operator running it by hand) | `pg_try_advisory_lock(hashtext('document-backup:<orgId>'))`, non-blocking; the second process logs `"org <id>: another backup is already running — skipped"` and moves to the next org. Key derivation is a pure one-argument function, so two processes cannot derive different keys (AD-007, the #163 lesson) | `T-U-05`, `T-M-05` |
...
| ST5 | Disk full (LOCAL) | Pre-flight: `statfs(BACKUP_DIR)` free bytes must exceed `sum(size) × 1.1`, else the run is refused **before** creating a run directory, recorded as `FAILED` with `"insufficient free space at the backup destination"`, and admins notified. If `ENOSPC` still occurs mid-copy, the run aborts, the partial run directory is removed, status is `FAILED`, and **no pruning happens** — a full disk must never be "solved" by deleting the backups we still have | `T-U-12`, `T-M-09` |
...
| ST7 | Destination unreachable (S3 down, `BACKUP_DIR` unmounted) | First `writeObject` failure that is not per-file (i.e. a connection/permission class error) aborts that org's run: status `FAILED`, sanitized reason, notify, continue to the next org. Exit code 1 so cron's log shows it | `T-U-13` |
...
| ST9 | Backup config missing for an org | Treated as `enabled: false` — no row is auto-created by the script. Only the settings page creates one (upsert). An org that never visited the page is never backed up, and the page says so | `T-M-02` |
...
|---|---|---|
| M1 | Sign out. Sign in as an **HR Admin**. Open `/settings` | The settings grid renders and contains **no** tile titled `Document Backup`. Then navigate directly to `/settings/backup`: the page body shows the app's 403 error text `Insufficient permissions` |
| M2 | Sign back in as **Super Admin**. Open `/settings`. Click the tile **Document Backup** | The page heading reads `Document Backup`. The `Enabled` switch is **off**. `Run every` shows `1` day(s). `Keep last` shows `7`. `Destination` shows `Local directory`. The run-history area shows the empty-state title `No backups yet` |
| M3 | Set `Enabled` on, `Run every` = `1`, `Keep last` = `3`, `Destination` = `Local directory`. Click **Save changes** | A green toast appears in the top-right reading `Backup settings saved`. The Save button is disabled while in flight (`Saving…`). After reload, the four controls still read `on / 1 / 3 / Local directory`. In psql: `select enabled, "intervalDays", "retentionCount" from backup_configs;` returns exactly `t / 1 / 3` |
| M4 | In a terminal: `BACKUP_DIR=$PWD/backups pnpm exec dotenv -e .env.dev -- tsx scripts/backup-documents.ts` | stdout contains a line `org org_veent: N file(s) copied, 0 failed`. `ls backups/org_veent/` lists exactly one directory named like `2026-08-22T…Z`. `cat backups/org_veent/*/manifest.json \| grep TRIPWIRE-164-MARKER` prints the marker's manifest entry, and that entry's `sha256` equals `sha256sum` of the corresponding file under `backups/org_veent/*/files/`. The marker's `fileName` in the manifest is the **original** upload filename, not the UUID on disk |
| M5 | Reload `/settings/backup` | The run-history table now has exactly one row. Its `Started` cell shows today's date/time, `Status` reads `Success`, `Files` shows the same N from M4, and `Destination` reads `Local directory`. The `Error` cell is empty |
| M6 | Re-run the exact command from M4 immediately | stdout contains `org org_veent: not due (next run …)`. `ls backups/org_veent/ \| wc -l` still prints `1`. The history table on reload still shows exactly one row — proving the interval gate, not just the cron schedule |
| M7 | Run with `--force` four times in a row, then reload the page | `ls backups/org_veent/ \| wc -l` prints `3`, not `5` — retention `Keep last 3` pruned the two oldest. The three surviving directories are the three most recent by name. The history table shows 5 rows (the record of every run is kept) and each of the 3 newest corresponds to a surviving directory |
| M8 | Break one file: `mv uploads/employees/<id>/<the marker's uuid>.pdf /tmp/`. Run with `--force`. Then reload the page | stdout reads `1 failed`. The newest history row's `Status` cell reads `Partial` (amber), `Failed` shows `1`, and the `Error` cell shows a short reason containing **no** absolute path. Open the bell/notification menu: a notification reads `Nightly document backup finished with errors (1 of N files could not be copied). Open Settings → Document Backup.` and links to `/settings/backup`. Restore the file: `mv /tmp/<uuid>.pdf uploads/employees/<id>/` |
| M9 | Set `BACKUP_DIR` inside the upload dir and run: `BACKUP_DIR=$PWD/uploads/bk pnpm exec dotenv -e .env.dev -- tsx scripts/backup-documents.ts` | The process exits non-zero **before** any org is processed, printing a message containing `backup destination must not be inside UPLOAD_DIR`. `ls uploads/` shows **no** `bk` directory was created, and `find uploads -type f \| wc -l` is unchanged |
| M10 | `ls -ld backups/org_veent/*/` and `ls -l backups/org_veent/*/manifest.json`; then re-run the tripwire `find uploads -type f -exec sha256sum {} \; \| sort` and diff against `tripwire-before.txt` | Directory mode is `drwx------` (0700) and `manifest.json` is `-rw-------` (0600). The `uploads` hash listing is **byte-identical** to the pre-change snapshot — proof that a full backup run read everything and changed nothing (G6/ST6) |
...
| AC-7 | A crashed or in-flight run can never be counted as a keeper by retention | `T-U-04`, `M5` | Fully-Automated + Hybrid |
| AC-8 | Two concurrent runs for one org cannot interleave, and both derive the identical lock key | `T-U-05`, `T-U-06`, `M5` | Fully-Automated + Hybrid |
